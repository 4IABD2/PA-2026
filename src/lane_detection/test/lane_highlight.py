...
class LaneDetector:
    def __init__(self, image_width=800, image_height=600):
        ...
        self.centers = np.linspace(CENTER_MIN * self.w, CENTER_MAX * self.w, CENTER_STEPS)
    ...
    def detect(self, bgr):
        ...
        result = {"mask": mask, "status": "NO_LANE", "score": 0.0,
                  "left": None, "right": None, "center_line": None,
                  "trajectory": None, "offset": 0.0, "direction": "NONE"}
        ...
        result.update({
            "status": "OK", "left": left, "right": right, "center_line": center_line,
    ...
    def _trajectory(self, cxs, aim_x, aim_y):
        ...
        ctrl = np.array([self.w / 2.0, (car[1] + target[1]) / 2.0])  # garde le cap puis tourne
...
def draw_overlay(bgr, result):
    ...
    if status == "OK":
        ...
        cv2.putText(overlay, f"offset={result['offset']:+.2f}  score={result['score']:.2f}",
                    (15, 70), FONT, 0.7, (255, 255, 255), 2, cv2.LINE_AA)
    else:
