...
def save_frame_data(frame_id, rgb_dir, canny_dir, overlay_dir, frame, result, labels):
    ...
    overlay = draw_overlay(frame, result, labels)

    cv2.imwrite(rgb_path, frame)
    cv2.imwrite(canny_path, result["mask"])
    ...
    return {
        ...
        "is_junction": bool(labels["is_junction"]),
        "lane_status": result["status"],
    }


def log_frame_info(frame_id, num_frames, labels, status):
    if frame_id % 50 == 0:
        print(
...
def generate_dataset(output_dir, num_frames):
    ...
    detector = LaneDetector(IMAGE_WIDTH, IMAGE_HEIGHT)
    ...
    try:
        ...
        while frame_id < num_frames:
            ...
            # Condition d'intersection DESACTIVEE (demande de Karim) : la
            # detection tourne partout. is_junction reste loggue dans le CSV
