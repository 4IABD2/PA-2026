...
**⚠️ Recadrage par l'éval déterministe de v15** (`runs/2026-07-15_03-36_ppo_v15_gtlane_60k/eval_model_final.json`) : la policy déterministe de v15 **ne bouge quasiment pas** (`pct_moving=0`, steer collapsé ~-0.8, frein net > accélérateur). Les +16.9/22 % de l'entraînement venaient du **bruit d'exploration**, pas de la policy apprise. Cause : la perception vérité-terrain a rendu « rester immobile centré sur la route » rentable (r_center+r_alive ≈ annulent le stall) → **optimum local passif**. Le bearing-to-goal seul ne fera pas bouger une policy passive.

**Second volet ajouté à v16 — gate anti-passivité** (`reward_fn.py`) : `r_center` n'est plus versé que si `is_on_road` **ET** `speed_kmh ≥ 1.0` (même seuil que r_stall). Rester immobile centré passe ainsi de ≈+0.335/step (r_center+r_alive, quasi break-even avec le stall) à `r_alive − r_stall` < 0 dès le premier step → l'immobilité cesse d'être un refuge. Justifié directement par l'éval : récompenser le centrage à l'arrêt était le bug. 1 test cassé recalculé + 3 mis à jour (les tests de centrage tournent désormais à 18 km/h pour que r_center s'active), 1 nouveau `test_r_center_gated_on_motion`. **282 tests verts**.

**Résultat v16** (run 40k, `runs/2026-07-15_06-42_ppo_v16_bearing_gate_40k/ANALYSIS.md`) — **la voiture conduit enfin en déterministe.**
- Entraînement (stochastique) : meilleur run du projet, succès 2 %→31 % par bloc, crash 98 %→66 %, reward -25→+15, 19 % de destinations globales (2× v15).
- **Éval déterministe (le vrai juge, leçon v15) : transformation nette.** `pct_moving` 0 %→**90 %**, distance parcourue 0.1 m→**35.9 m** (jusqu'à 87 m), **steer désaturé** (~0.0 ± 0.9 vs -0.8 collapsé en v15 — valide enfin le `squash_output` de v13 : le collapse était lié à la passivité, pas au squash), throttle passe du frein net à +0.65, **0 crash** sur les 13 scénarios. **1/13 succès formel** (v15 : 0).
...
**Résultat v17** (run 80k, `runs/2026-07-15_09-50_ppo_v17_routeaware_80k/ANALYSIS.md`) — **meilleur run du projet, fix validé, mais navigation encore imparfaite** :
- **Entraînement** : reward -50 → +59 (record du projet), **3 avancements de curriculum** (30→38→46→54 m, mécanisme jamais déclenché avant v17 — la policy franchit 50 % de succès à des paliers de plus en plus durs). Preuve d'un apprentissage qualitativement différent.
- **Éval déterministe** : **2/13** succès (v16 : 1/13, v15 : 0/13), 80 % moving, 33 m moyens, **0 crash**. **`curve_left` : bloqué (0.7 m) en v16 → SUCCÈS (25.4 m) en v17** — validation directe du fix (le scénario que le guidage à vol d'oiseau cassait). 2ᵉ succès : `npc_crossing`.
