...
**Prochaine étape** : smoke CARLA `--ground-truth-lane` puis run v16 60k `--ground-truth-lane` (= bearing-to-goal + gate anti-passivité, sur base v15). Métriques à surveiller : **la policy déterministe bouge-t-elle enfin** (à vérifier par une éval de fin, pas seulement le CSV stochastique qui masque la passivité) ? Le taux de succès *déterministe* décolle-t-il ? Si l'immobilité persiste malgré le gate → le problème est plus profond (steer collapsé à -0.8 non résolu par squash ; l'exploration gSDE/ent_coef élevé porte tout le poids pendant que la moyenne reste passive) → pistes : baisser ent_coef/gSDE pour forcer la moyenne à conduire, ou reprendre le pont CIL→RL (warmstart par imitation) abandonné en début de projet, dont la prémisse (« PPO from scratch converge en heures ») est réfutée par toute la série v9→v16.
